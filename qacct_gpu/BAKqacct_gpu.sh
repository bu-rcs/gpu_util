...
qacct -j $1 | grep -E $fields > output
...
# Currently this ignore the possibility of a job that take place over three distinct months
...
if [ $startdate -eq $enddate ]; then
	...
	cat gpustats/$startdate $enddate > logs
fi
grep $1 logs > job
